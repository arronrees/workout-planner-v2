...
import { createWorkoutModel, recordWorkoutModel } from '@/models/workouts';
import { z } from 'zod';
...
export async function checkRecordWorkoutObjectValid(workout: any): Promise<{
  success: boolean;
  error: string | null;
}> {
  try {
    recordWorkoutModel.parse(workout);
